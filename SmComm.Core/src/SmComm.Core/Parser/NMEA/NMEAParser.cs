...
namespace SmComm.Core.Parser.NMEA
{
    ...
    /// version info
    /// 0.1 - full lists NMEA 0183 2.0 talkers IDs and sentences IDs, Garmin proprietary sentenses
    /// 0.2 - Martech proprietary sentences added, Datum dictionary added
    ...
    public enum ManufacturerCodes
    {
        ...
        PMI,
        PMP,
        PPR, // Added duplicate for NORTEK since PPR was in sample Bozon setup
        PRK,
        PSM,
        PTC,
        PTG,
    ...
    public static class NMEAParser
    {
        ...
        private static Dictionary<ManufacturerCodes, Dictionary<string, string>> ProprietarySentencesFormats =
            new Dictionary<ManufacturerCodes, Dictionary<string, string>>()
            {
                ...
                {
                    ManufacturerCodes.NOR,
                    new Dictionary<string, string>()
                    {
                       {"I", "0=Aquadopp|2=Aquadopp Profiler|3=AWAC,aaannnn,x,x,x.x,x.x,0=ENU|1=XYZ|2=Beam"}, // Information (configuration)
                       {"S", "x,x,hh,hh,x.x,x.x,x.x,x.x,x.x,x.x,x.x,x,x"}, // Sensor Data
                       {"C", "x,x,x,x.x,x.x,x.x,x.x,x.x,c--c,c--c,c--c,c--c,c--c,c--c,c--c"}, // Current velocity data
                       {"W", "x,x,0=pressure|1=velocity|3=AST,1=PUV|2=SUV|3=MLM|4=MLMST,x.x,x.x,x.x,x.x,x.x,x.x,x.x,x.x,x.x,x.x,x.x,x.x,x,x,x.x,x.x,hhh"}, // Wave parameters
                       {"E", "x,x,0=pressure|1=velocity|3=AST,x.x,x.x,n,[x.x]"}, // Wave energy density spectrum
                       {"B", "x,x,0=pressure|1=velocity|3=AST,1=PUV|2=SUV|3=MLM|4=MLMST,x.x,x.x,x.x,x.x,x.x,x.x,x.x,x.x,hhhh"}, // Wave band parameters
                       {"F", "CC,x,x,0=pressure|1=velocity|3=AST,x.x,x.x,x,[x.x]"}, // Fourier coefficient spectra
                    }
                },
        ...
        private static Dictionary<string, Func<string, object>> parsers = new Dictionary<string, Func<string, object>>()
        {
            ...
            { "hhmmss.ss", x => ParseCommonTime(x) },
            { "ddmmyy", x => ParseCommonDate(x) },
            { "mmddyy", x => ParseCommonDate(x,"mmddyy") },
            { "dddmm.mmm", x => ParseCommonDegrees(x) }
        };
        ...
        private static Dictionary<string, Func<object, string>> formatters = new Dictionary<string, Func<object, string>>()
        {
            ...
            { "hhmmss.ss", x => FormatTime((DateTime)x, true) },
            { "ddmmyy", x => FormatDate((DateTime)x) },
            { "mmddyy", x => FormatDate((DateTime)x, "mmddyy") },
            { "dddmm.mmm", x => FormatDegree((double)x) }
        };
#endif
        ...
        private static string FormatDate(DateTime dt, string format ="ddmmyy")
        {
            StringBuilder sb = new StringBuilder();
            if (format == "ddmmyy")
            {
        ...
        {            
            ...
            bool ok = source.StartsWith(SentenceStartDelimiter.ToString());
            if (!ok)
            {
                Debug.WriteLine("ok false");
            ...
            try
            {
                ...
                if (!string.IsNullOrEmpty(checkSumString))
                {
                    ...
                    if (byte.TryParse(checkSumString, System.Globalization.NumberStyles.HexNumber, CultureInfo.InvariantCulture, out declaredCheckSum))
                    {
                        ...
                        {
                            throw new ArgumentException(string.Format("Check sum error in sentese \"{0}\" (declared: {1}, real: {2})", sentence, declaredCheckSum, realCheckSum));
                        }
                    }
                    else
                    {
        ...
        private static NMEAStandardSentence ParseSentence(TalkerIdentifiers talkerID, string sentenceIDString, List<string> parameters)
        {
            ...
            {
                throw new ArgumentException(string.Format("Undefined sentence ID \"{0}\" from talker \"{1}\"", sentenceIDString, talkerID));
            }
            ...
            if (splits.Length > 1)
            {
                ...
                if (sentenceDescription.Length >= 4)
                {
                    ...
                    else
                    {
                        ...
                        {
                            throw new ArgumentException(string.Format("Undefined talker ID \"{0}\"", talkerIDString));
                        }
        ...
        private static object[] ParseParameters(List<string> parameters, string formatString)
        {
            ...
            if ((formatTokens.Length >= parameters.Count) || (formatString.Contains(etcFormat)) || formatString.Contains(arrayOpenBracket))
            {
        ...
        private static DateTime ParseCommonDate(string token, string format="ddmmyy")
        {
            if (token.Length == 6 && format == "ddmmyy")
            {
            ...
            else if (token.Length == 6 && format == "mmddyy")
            {
                int month = Convert.ToInt32(token.Substring(0, 2));
                int date = Convert.ToInt32(token.Substring(2, 2));
                int year = Convert.ToInt32(token.Substring(4, 2)) + 2000;
